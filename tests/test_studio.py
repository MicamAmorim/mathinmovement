...
class StudioAssetTests(unittest.TestCase):
    def test_studio_assets_exist_and_reference_core_endpoints(self):
        ...
        self.assertIn('accept=".demo,.qenem" multiple', index)
        self.assertIn('id="toggleJobHistory"', index)
        self.assertIn('id="jobsPageSize"', index)
        self.assertIn("processed-pill", index)
        self.assertIn('request("/contents")', script)
        self.assertIn('request("/imports/batch"', script)
        ...
        self.assertIn('request("/jobs?limit=5")', script)
        self.assertIn("/jobs/history?limit=", script)
        self.assertIn('method: "POST"', script)
        self.assertIn(".catalog-grid", style)
        self.assertIn(".job-status", style)
        self.assertIn(".processed-pill", style)
        self.assertIn(".jobs-history-controls", style)
